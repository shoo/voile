 ...
 * JSONValueデータを得る
 */
JSONValue json(T)(T[] x) @property /+pure+/
	if (isSomeString!(T[]))
{
	JSONValue v;
...
/// ditto
JSONValue json(T)(T[] ary) @property /+pure+/
 ...
 * JSONValueデータの操作
 */
void setValue(T)(ref JSONValue v, string name, T val) /+pure+/
	if (is(typeof(val.json)))
	body
{
	v.object[name] = val.json;
}
...
void setValue(T)(ref JSONValue v, string name, T val) /+pure+/
	if (is(T == enum))
	in { assert(v.type == JSON_TYPE.OBJECT); }
	body
{
	v.object[name] = to!string(val).json;
}
...
/// ditto
T getValue(T)(ref JSONValue v, string name, T defaultVal = T.init) /+pure+/
	if (isSomeString!(T))
{
	T ret = defaultVal;
