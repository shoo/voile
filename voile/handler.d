...
private:
	...
	template _EmitGeneratingPolicy()
	{
		template generateFunctionBody(unused...)
		{
			...
			q{
				alias FnTy = SetFunctionAttributes!(typeof((*cast(Unique!ProcList*)&_procs)[].front), _exFuncInfo.linkage, _exFuncInfo.attrib);
				static if (is(_exFuncInfo.RT: bool))
				{
	...
	static if (Args.length > 0 && !hasUnsharedAliasing!Args)
	{
		...
		static Proc toConnectable(Range)(Range r)
			...
			&& (is(Range == class)
			 || is(Range == interface)
			 || isPointer!Range)
			&& is(typeof({ Range tmp; if (!tmp.empty) assert(1); })))
		{
